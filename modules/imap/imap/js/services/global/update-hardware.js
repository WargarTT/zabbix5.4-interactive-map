...
const makeLi = (hostId, name, image, onSet) => {
    ...
    li.innerHTML = `<img alt="" width="20px" src="modules/imap/imap/hardware/${image}"> ${name ? name : 'none'}`;
...
class UpdateHardwareService {
    ...
    setHardware(host, value) {
        setHardwareType(host.hostid, value)
            .then(_ => {
                if (typeof overlayDialogueDestroy === 'function') {
