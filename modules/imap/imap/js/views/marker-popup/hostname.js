class HostNameElement {
    ...
    refresh(host) {
        ...
        if (host.hardware && host.hardware !== 'undefined' && this.showIcons) {
            let image = L.DomUtil.create('img', 'hardwareIcon', this.element);
            image.onerror = () => {
                image.src = 'modules/imap/imap/hardware/none.png';
            };
            image.setAttribute('title', host.hardware);
            image.setAttribute('src', `modules/imap/imap/hardware/${host.hardware}.png`);
        }
